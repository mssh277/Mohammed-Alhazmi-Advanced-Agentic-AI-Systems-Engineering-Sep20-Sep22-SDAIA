...
def collect_preflight(root: Path = ROOT) -> dict[str, Any]:
    ...
    if (root / "reports/submission_manifest.json").is_file():
        # A completed submission must retain executed outputs. Validate the
        # full submission contract instead of the cleared teaching template.
        from validate_submission import validate_submission
        submission_report = validate_submission(root)
        notebook_errors = submission_report["failure_messages"]
        if not submission_report["all_passed"] and not notebook_errors:
            notebook_errors = ["Completed submission validation failed"]
    else:
        notebook_errors = validate_notebook(root / NOTEBOOK.relative_to(ROOT))
    add(
        "notebook_contract",
        not notebook_errors,
        "Teaching templates require cleared outputs; completed submissions require the full validated evidence contract.",
