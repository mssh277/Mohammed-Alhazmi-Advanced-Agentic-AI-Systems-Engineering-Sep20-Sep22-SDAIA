# Rafeeq Mini learning progress

| Field | Value |
|---|---|
| Public learner ID | mssh277 |
| Course | Advanced Agentic AI Systems Engineering |
| Programme | SDA-AIE-311 |
| Project | Rafeeq Mini |

## Checkpoints

| Stage | Public status | Evidence kept safely |
|---|---|---|
| Setup | IN PROGRESS | Repository and Drive notebook ready; email verification awaiting learner confirmation. |
| Day 1 / C9_DAY1_GATE | PASS | Generated evidence retained in the saved Drive notebook. |
| Day 2 / C20_DAY2_GATE | PENDING | Keep generated evidence in the saved Drive notebook. |
| Day 3 / C29_EXPORT_SAFETY_CHECK | PENDING | Upload only after FINAL_EXPORT_CREATED. |

## Safe-update rule

Before C29, update only this progress log. Do not upload source code, the live notebook, raw outputs, traces, temporary checkpoints, credentials, private links, personal data, or ZIP files. Keep the working evidence in Drive until the final clean export.
